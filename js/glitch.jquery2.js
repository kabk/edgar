...
(function($){
    $.fn.glitch = function(options) {
        var s = $.extend({
            ...
            maxint: 0,     // max interval between glitchings
            minint: 0,      // min interval between glitchings
            maxglitch: 1111,   // max number of twitches
            hshift: 8,      // max horizontal shift 
            vshift: 8,      // max vertical shift
            direction: 'random' // 'horizontal', 'vertical' or 'random'
        }, options);

        return this.each(function(){
            $t=$(this);
            $t.wrap('<span style="display:inline-block;position:relative;z-index:0;">');
